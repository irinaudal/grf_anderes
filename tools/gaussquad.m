...
else  %upper integral limit is a VECTOR of limits
    %rewrite the base points and weights to handle array of [length(x) x length(b)]
   [bg xg] = meshgrid(b,x);
   [wg wg] = meshgrid(b,w);
   
   % Linearly transform from [-1,1] to [a,b].
   x = (bg - a) / 2 .* xg + (a + bg) / 2;   % transform base points X
   w = (bg - a) / 2 .* wg;                 % adjust weigths
